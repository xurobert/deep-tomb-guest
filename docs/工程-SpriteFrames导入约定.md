...
| 目录 | 内容 | 尺寸 |
|------|------|------|
| `characters/hero/` | 主角地图精灵 | 64×64 px |
| `characters/enemies/` | 敌人地图精灵 | 64×64 px |
| `characters/npc/` | NPC 地图精灵 | 64×64 px |
...
| 资产类型 | 画布尺寸 |
|----------|----------|
| 角色精灵 (character sprite) | 64×64 px |
...
| 设置项 | 值 | 说明 |
|--------|-----|------|
| Filter | **OFF** (Nearest) | 最近邻插值，保持像素锐利 |
| Mipmaps | **OFF** | 禁用 Mipmap |
| 缩放算法 | **最近邻（Nearest Neighbor）** | 禁止双线性/双三次 |

**⚠️ 严禁**：将 Grok 生成的高清图（如 1280×720）直接导入 SpriteFrames。
