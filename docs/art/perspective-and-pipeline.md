...
| 资源类型 | 尺寸 | 说明 |
|----------|------|------|
| 地图瓦片 (map-tile) | **32×32 px** | 整格拼接，无缝边缘（~~旧 16×16~~） |
| 地图角色精灵 (map-sprite) | **64×64 px** | 探索/战斗场景中显示（~~旧 32×32~~） |
...
| 分辨率 | **64×64 px**（~~旧 32×32~~） | 高（512+ px，或按需） |
| 风格 | 像素美术（硬边缘、有限调色板） | 日系立绘风格 |
...
- ❌ 自由视角旋转
- ❌ 非整数缩放导致的像素模糊
- ❌ 混用不同像素密度的资源（如 64×64 精灵放在 16×16 瓦片上）
